...
/// Region information for AVC420 bitmap stream
#[derive(Debug, Clone)]
pub struct Avc420Region {
    ...
    /// Top edge of region
    pub top: u16,
    /// Right edge of region (inclusive)
    pub right: u16,
    /// Bottom edge of region (inclusive)
    pub bottom: u16,
    /// Quantization parameter (0-63)
    pub qp: u8,
    /// Progressive mode flag
...
/// Create AVC420 bitmap stream data for MS-RDPEGFX
///
/// This wraps H.264 NAL data in the RFX_AVC420_BITMAP_STREAM format
/// required by the RDPEGFX protocol.
///
/// # Format (per MS-RDPEGFX 2.2.4.3)
///
/// ```text
/// numRegionRects: u32
/// regionRects: [RDPGFX_RECT16; numRegionRects]  // 8 bytes each
/// quantQualityVals: [RFX_AVC420_QUANT_QUALITY; numRegionRects]  // 2 bytes each
/// avc420EncodedBitstream: [u8]  // Length-prefixed NAL units
/// ```
///
/// # Arguments
///
/// * `h264_data` - H.264 bitstream in AVC length-prefixed format
/// * `regions` - Region rectangles with quality parameters
///
/// # Returns
///
/// Serialized AVC420 bitmap stream ready for WireToSurface1Pdu
pub fn create_avc420_bitmap_stream(h264_data: &[u8], regions: &[Avc420Region]) -> Vec<u8> {
    let num_regions = regions.len() as u32;

    // Calculate total size:
    // - 4 bytes for numRegionRects
    // - 8 bytes per rectangle (left, top, right, bottom as u16)
    // - 2 bytes per quant/quality value
    // - H.264 data
    let header_size = 4 + (regions.len() * 8) + (regions.len() * 2);
    let mut output = Vec::with_capacity(header_size + h264_data.len());

    // Write numRegionRects (little-endian)
    output.extend_from_slice(&num_regions.to_le_bytes());

    // Write region rectangles (RDPGFX_RECT16: left, top, right, bottom as LE u16)
    for region in regions {
        output.extend_from_slice(&region.left.to_le_bytes());
        output.extend_from_slice(&region.top.to_le_bytes());
        output.extend_from_slice(&region.right.to_le_bytes());
        output.extend_from_slice(&region.bottom.to_le_bytes());
    }

    // Write quant/quality values (RFX_AVC420_QUANT_QUALITY)
    for region in regions {
        // Byte 1: bits 0-5 = QP, bit 7 = progressive flag
        let mut quant_byte = region.qp & 0x3F; // QP in bits 0-5
        if region.progressive {
            quant_byte |= 0x80; // Progressive flag in bit 7
        }
        output.push(quant_byte);
        // Byte 2: quality value
        output.push(region.quality);
    }

    // Write H.264 bitstream data
    output.extend_from_slice(h264_data);
...
mod tests {
    ...
    fn test_avc420_region_full_frame() {
        let region = Avc420Region::full_frame(1920, 1080, 26);
        assert_eq!(region.left, 0);
        assert_eq!(region.top, 0);
        assert_eq!(region.right, 1919);
        assert_eq!(region.bottom, 1079);
        assert_eq!(region.qp, 26);
        assert!(!region.progressive);
        assert_eq!(region.quality, 100);
    }

    #[test]
    fn test_create_avc420_bitmap_stream_single_region() {
        let h264_data = vec![0x00, 0x00, 0x00, 0x04, 0x67, 0x42, 0x00, 0x1e];
        let regions = vec![Avc420Region::full_frame(64, 64, 26)];

        let stream = create_avc420_bitmap_stream(&h264_data, &regions);

        // Expected layout:
        // - 4 bytes: numRegionRects = 1 (LE)
        // - 8 bytes: rectangle (left=0, top=0, right=63, bottom=63)
        // - 2 bytes: quant/quality (qp=26, quality=100)
        // - 8 bytes: H.264 data
        assert_eq!(stream.len(), 4 + 8 + 2 + 8);

        // Check numRegionRects
        assert_eq!(&stream[0..4], &[0x01, 0x00, 0x00, 0x00]);

        // Check rectangle (left=0, top=0, right=63, bottom=63)
        assert_eq!(&stream[4..6], &[0x00, 0x00]); // left
        assert_eq!(&stream[6..8], &[0x00, 0x00]); // top
        assert_eq!(&stream[8..10], &[0x3F, 0x00]); // right = 63
        assert_eq!(&stream[10..12], &[0x3F, 0x00]); // bottom = 63

        // Check quant/quality (qp=26 in bits 0-5, progressive=false)
        assert_eq!(stream[12], 26); // QP
        assert_eq!(stream[13], 100); // Quality

        // Check H.264 data preserved
        assert_eq!(&stream[14..], &h264_data);
    }

    #[test]
    fn test_create_avc420_bitmap_stream_progressive() {
        let h264_data = vec![0x01, 0x02];
        let regions = vec![Avc420Region {
            left: 0,
            top: 0,
            right: 15,
            bottom: 15,
        ...
        // Check quant byte has progressive flag set (bit 7)
        assert_eq!(stream[12], 30 | 0x80); // QP=30 + progressive bit
        assert_eq!(stream[13], 80); // Quality
    }
}
