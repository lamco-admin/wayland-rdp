...
pub fn format_user_error(error: &anyhow::Error) -> String {
    ...
    writeln!(&mut output, "  - Run with --verbose for detailed logs: wrd-server -vvv").ok();
    writeln!(&mut output, "  - Check logs in: /var/log/wrd-server/").ok();
    writeln!(&mut output, "  - Report issues: https://github.com/lamco-admin/wayland-rdp/issues").ok();
    writeln!(&mut output, "╚════════════════════════════════════════════════════════════╝").ok();
...
fn format_portal_error(output: &mut String, error: &str) {
    ...
    writeln!(output, "").ok();
    writeln!(output, "Could not access the screen capture system (xdg-desktop-portal).").ok();
    writeln!(output, "").ok();
    writeln!(output, "Common Causes:").ok();
    writeln!(output, "").ok();
    writeln!(output, "  1. Portal permission denied").ok();
    writeln!(output, "     → When dialog appears, click 'Allow' or 'Share'").ok();
    writeln!(output, "     → Run the server again if you clicked 'Deny'").ok();
    writeln!(output, "").ok();
    writeln!(output, "  2. Portal is not running").ok();
    writeln!(output, "     → Run: systemctl --user status xdg-desktop-portal").ok();
    writeln!(output, "     → If not running: systemctl --user start xdg-desktop-portal").ok();
    writeln!(output, "").ok();
    writeln!(output, "  3. Portal backend not installed").ok();
    writeln!(output, "     → For GNOME: sudo apt install xdg-desktop-portal-gnome").ok();
    writeln!(output, "     → For KDE: sudo apt install xdg-desktop-portal-kde").ok();
    writeln!(output, "     → For wlroots: sudo apt install xdg-desktop-portal-wlr").ok();
    writeln!(output, "").ok();
    writeln!(output, "  4. Not running in Wayland session").ok();
    writeln!(output, "     → Check: echo $WAYLAND_DISPLAY (should not be empty)").ok();
    writeln!(output, "     → Log out and select 'Wayland' session at login").ok();
}
...
fn format_pipewire_error(output: &mut String, error: &str) {
    ...
    writeln!(output, "     → If not running: systemctl --user start pipewire").ok();
    writeln!(output, "     → Also start: systemctl --user start wireplumber").ok();
    writeln!(output, "").ok();
    ...
    writeln!(output, "     → Upgrade: sudo apt update && sudo apt upgrade pipewire").ok();
    writeln!(output, "").ok();
    writeln!(output, "  3. No permission to access video").ok();
    writeln!(output, "     → Add your user to video group: sudo usermod -aG video $USER").ok();
...
fn format_tls_error(output: &mut String, error: &str) {
    ...
    writeln!(output, "").ok();
    writeln!(output, "Could not load TLS certificates for secure connections.").ok();
    writeln!(output, "").ok();
    writeln!(output, "Common Causes:").ok();
    writeln!(output, "").ok();
    ...
    writeln!(output, "       openssl req -x509 -newkey rsa:4096 -nodes \\").ok();
    writeln!(output, "         -keyout certs/key.pem -out certs/cert.pem \\").ok();
    ...
    writeln!(output, "     → Check file starts with '-----BEGIN CERTIFICATE-----'").ok();
}

fn format_network_error(output: &mut String, error: &str) {
    writeln!(output, "Network Binding Error").ok();
    writeln!(output, "").ok();
    writeln!(output, "Could not bind to network address for RDP connections.").ok();
    writeln!(output, "").ok();
    writeln!(output, "Common Causes:").ok();
    writeln!(output, "").ok();
    ...
    writeln!(output, "     → Change in config.toml: listen_addr = '0.0.0.0:3390'").ok();
    writeln!(output, "").ok();
...
fn format_config_error(output: &mut String, error: &str) {
    ...
    writeln!(output, "     → Default location: /etc/wrd-server/config.toml").ok();
    writeln!(output, "     → Or specify: wrd-server -c /path/to/config.toml").ok();
    writeln!(output, "     → Create from example: cp config.toml.example config.toml").ok();
    writeln!(output, "").ok();
...
fn format_generic_error(output: &mut String, error: &str) {
    ...
    writeln!(output, "     → echo $XDG_SESSION_TYPE (should be 'wayland')").ok();
    writeln!(output, "").ok();
